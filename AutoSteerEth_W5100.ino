...
byte relay = 0, speeed = 0, workSwitch = 0, steerSwitch = 1, switchByte = 0, steerMode = 0;
...
static byte myip[] = { 192,168,100,98 };
// gateway ip address
...
//byte Ethernet::buffer[700]; // udp send and receive buffer
...
void loop()
{
    ...
    if(UdpSEND.beginPacket(ipDestination, portDestination));{
      UdpSEND.write(toSend, sizeof(toSend));
      UdpSEND.endPacket();
    }
  ...
  if (packetSize) {
    UdpRECEIVE.read(packetBuffer,packetSize);

    udpSteerRecv(localPort, UdpRECEIVE.remoteIP(), UdpRECEIVE.remotePort(), packetBuffer, packetSize);
  }
...
void udpSteerRecv(uint16_t dest_port, uint8_t src_ip[IP_LEN], uint16_t src_port, byte *data, uint16_t len)
{
  ...
  //Serial.print("  sPort: ");  Serial.println(src_port); 
  ...
  //for (int i = 0; i < len; i++) {
    ...
    if (data[0] == 0x7F && data[1] == 0xFE) //Data
    {
      ...
      abHeading = ((int)(data[8] << 8 | data[9])); //high,low bytes
      Serial.print(F("AB Heading: "));
      Serial.println(abHeading, DEC);
    ...
    if (data[0] == 0x7F && data[1] == 0xF4)
    {
      steerMode = data[2]; //read steering sensor mode from AgOpenGPS
